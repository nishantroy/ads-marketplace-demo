...
This branch (`api-integration`) uses committed main `6c6888e`. Engine and contract changes in progress on
main are deliberately not copied; reconcile them before integrating. `src/lib/server/engine-adapter.ts`
is the only API module importing engine/fixture implementations. UI remains in `ui-workspace`.
