...
export function RequestFunnel({ trace, scenario }: { trace: RequestTrace; scenario: ScenarioSummary }) {
  ...
  const finalists = trace.candidates.filter(c => c.ranking.evaluated && c.ranking.shortlisted);
  ...
  const outsideShortlist = scored.filter(c => c.scoring.evaluated && c.scoring.passedThreshold && !(c.ranking.evaluated && c.ranking.shortlisted)).length;
  ...
  const stages: Stage[] = [
    ...
    {
      title: "Ranking finalists", explanation: `Keep campaigns above the minimum quality, then take the top ${scenario.config.shortlistSize} by utility (effective bid × quality). Quality is not a bid.`,
      ...
      rows: scored.map(c => row(c, c.ranking.evaluated && c.ranking.shortlisted,
